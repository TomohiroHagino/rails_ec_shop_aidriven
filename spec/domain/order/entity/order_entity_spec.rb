...
RSpec.describe Domain::Order::Entity::OrderEntity do
  let(:order_id) { Domain::Order::ValueObject::OrderId.new(1) }
  let(:user_id) { Domain::User::ValueObject::UserId.new(1) }
  let(:total_amount) { Domain::Product::ValueObject::Price.new(5000) }
  let(:status) { Domain::Order::ValueObject::OrderStatus.new('pending') }
  let(:order) do
    described_class.new(
      id: order_id,
      user_id: user_id,
