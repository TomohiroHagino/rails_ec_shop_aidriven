...
RSpec.describe Domain::UserAggregate::Entity::UserEntity do
  let(:user_id) { Domain::UserAggregate::ValueObject::UserId.new(1) }
  let(:email) { Domain::UserAggregate::ValueObject::Email.new('test@example.com') }
  let(:user) do
    described_class.new(
      id: user_id,
      name: 'テストユーザー',
  ...
  describe '#change_email' do
    it 'メールアドレスを変更できる' do
      new_email = Domain::UserAggregate::ValueObject::Email.new('new@example.com')
  ...
  describe '等価性' do
    ...
    it '異なるIDのユーザーは等しくない' do
      user1 = described_class.new(
        id: Domain::UserAggregate::ValueObject::UserId.new(1),
        name: 'テスト',
        email: email,
        password_digest: 'password'
      )
      user2 = described_class.new(
        id: Domain::UserAggregate::ValueObject::UserId.new(2),
        name: 'テスト',
        email: email,
        password_digest: 'password'
      )
